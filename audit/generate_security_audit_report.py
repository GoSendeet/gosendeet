...
MARKDOWN_PATH = ROOT / "security_audit_report.md"
PDF_PATH = ROOT / "security_audit_report.pdf"


REPORT = """# GoSendeet Security Audit
...
SEVERITY_COLORS = {
    "Critical": (0.73, 0.11, 0.11),
    "High": (0.82, 0.27, 0.10),
    "Medium": (0.85, 0.62, 0.13),
    "Low": (0.10, 0.53, 0.24),
}
...
NORMAL = Style("F1", 11, (0.12, 0.12, 0.12), 15)
SMALL = Style("F1", 9, (0.30, 0.30, 0.30), 12)
BOLD = Style("F2", 11, (0.10, 0.10, 0.10), 15)
H1 = Style("F2", 22, (0.05, 0.25, 0.13), 28)
H2 = Style("F2", 15, (0.05, 0.25, 0.13), 22)
...
def wrap_text(text: str, max_width: int, font_size: int) -> list[str]:
    if not text:
        return [""]
    approx_char_width = max(font_size * 0.52, 5.2)
    max_chars = max(int(max_width / approx_char_width), 20)
    words = text.split()
    lines: list[str] = []
    current = ""
    for word in words:
...
def add_text(
    ...
    r, g, b = style.color
    commands.append(f"BT /{style.font} {style.size} Tf {r:.3f} {g:.3f} {b:.3f} rg 1 0 0 1 {x} {y} Tm ({escape_pdf_text(text)}) Tj ET")
...
def build_pdf(markdown: str) -> bytes:
    ...
    for raw in lines:
        ...
        if line.startswith("# "):
            ensure_space(H1.leading + 6)
            add_text(current, line[2:].strip(), left, y, H1)
            y -= H1.leading + 4
            continue

        if line.startswith("### "):
            ensure_space(H2.leading + 4)
            add_text(current, line[4:].strip(), left, y, H2)
            y -= H2.leading
            continue

        if line.startswith("Severity: "):
            severity = line.split(":", 1)[1].strip()
            color = SEVERITY_COLORS.get(severity, (0.30, 0.30, 0.30))
            ensure_space(24)
            x = left
            box_width = 110
            box_height = 18
            rect_y = y - 5
            current.append(
                f"{color[0]:.3f} {color[1]:.3f} {color[2]:.3f} rg {x} {rect_y} {box_width} {box_height} re f"
            )
            add_text(current, f"Severity: {severity}", x + 8, y + 1, Style("F2", 10, (1, 1, 1), 12))
            y -= 24
            continue

        if re.match(r"^(Title|Affected component|Evidence|Description|Exploitation scenario|Impact|Recommended fix|Reference|Threat model|Primary trust boundaries|Sensitive assets|Chain|Architectural improvements|Safer patterns and best practices):", line):
            label, value = line.split(":", 1)
            wrapped = wrap_text(value.strip(), content_width - 5, NORMAL.size)
            ensure_space((len(wrapped) + 1) * NORMAL.leading)
            add_text(current, f"{label}:", left, y, BOLD)
            y -= NORMAL.leading
            for wrapped_line in wrapped:
                add_text(current, wrapped_line, left + 12, y, NORMAL)
                y -= NORMAL.leading
            continue

        if line.startswith("- "):
            wrapped = wrap_text(line[2:].strip(), content_width - 18, NORMAL.size)
            ensure_space(len(wrapped) * NORMAL.leading)
            first = True
            for wrapped_line in wrapped:
                prefix = "• " if first else "  "
                add_text(current, f"{prefix}{wrapped_line}", left, y, NORMAL)
                y -= NORMAL.leading
                first = False
            continue

        if re.match(r"^\d+\.", line):
            wrapped = wrap_text(line, content_width, NORMAL.size)
            ensure_space(len(wrapped) * NORMAL.leading)
            for wrapped_line in wrapped:
                add_text(current, wrapped_line, left, y, NORMAL)
                y -= NORMAL.leading
            continue

        style = SMALL if line.startswith("Audit date:") or line.startswith("Scope reviewed:") else NORMAL
        wrapped = wrap_text(line, content_width, style.size)
        ensure_space(len(wrapped) * style.leading)
        for wrapped_line in wrapped:
            add_text(current, wrapped_line, left, y, style)
            y -= style.leading
    ...
    font1_id = add_object(font1)
    font2_id = add_object(font2)

    page_refs: list[int] = []
    for page_commands in pages:
        safe_commands = [normalize_pdf_text(command) for command in page_commands]
        content_stream = "\n".join(safe_commands).encode("latin-1")
    ...
    pages_kids = " ".join(f"{page_id} 0 R" for page_id in page_refs)
    pages_id = add_object(f"<< /Type /Pages /Kids [{pages_kids}] /Count {len(page_refs)} >>")
    ...
    for index, obj in enumerate(objects, start=1):
        ...
        if index in page_refs:
            obj = obj.replace(b"/Parent 0 0 R", f"/Parent {pages_id} 0 R".encode("ascii"))
        raw.extend(obj)
        raw.extend(b"\nendobj\n")
